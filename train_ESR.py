...
# Networks
parser.add_argument('--model_type', type=str, default='EDSRx2v3')
parser.add_argument('--upscale_factor', type=int, default=2, help="super resolution upscale factor")
parser.add_argument('--prefix', default='edsrc2_v3', help='description of the model for weights')
# Pretrained Model
...
parser.add_argument('--test_dir', type=str, default='test/', help='test data')
# Loading Settings
...
# Output
parser.add_argument('--snapshots', type=int, default=5, help='Snapshots for checkpoint and rmse computing')
parser.add_argument('--save_folder', default='./weights/', help='Location to save checkpoint models')
parser.add_argument('--testBatchSize', type=int, default=1, help='testing batch size')
parser.add_argument('--out_rmse', default="./rmse.txt", help='path for rmse')
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        t0 = time.time()
        result1, result2 = model(input, input_rgb)
        t1 = time.time()

        loss = criterion(result1, target) + criterion(result2, target)# + 0.1*criterion(g1, g_gt) + 0.1*criterion(g2, g_gt)
...
def validate(epoch):
    ...
    with torch.no_grad():
        for batch in testing_data_loader:
            ...
            "Output"
            t0 = time.time()
            prediction = model(input, input_rgb)[0]
...
if opt.model_type == 'EDSRx2v3':
    model = EDSRx2v3(num_channels=1, base_filter=64,  feat = 256, num_stages=4, scale_factor=opt.upscale_factor)
